...
class PrivacyPolicyController extends Controller
{
    ...
    public function listPrivacyPolicy()
    {
       ...
       /* $modeldata = PrivacyPolicy::where('role','!=',ADMIN_ROLE)->orderBy('id','DESC')->get();
        return  View::make("admin.$this->model.index",compact('modeldata'));*/
    }
    /**
        * Function for display page  for add new PrivacyPolicy page 
        *
        * @param null
        *
        * @return view page. 
    */
    ...
    public function savePrivacyPolicy()
    {
    	...
    	$validator  =   Validator::make(
            ...
            array(
                ...
                'email'              => 'required|email|unique:PrivacyPolicy,email',
                'password'           => 'required',
        ...
        {   
            ...
            $PrivacyPolicyname = "demo";
            $full_name = "demo".' '."PrivacyPolicy";
            
            $subject = "PrivacyPolicy register successfully latest.";
            $messageBody = "Hello ".$PrivacyPolicyname."<br><br>".$full_name." has been register successfully in tutorial points.";
            $this->sendMail($email, $full_name, $subject, $messageBody, $settingsEmail);
             
        }     

        Session::flash('success',  trans("PrivacyPolicy has been added successfully."));  
        return Redirect::route("$this->model.index");
        
    }

    /*delete PrivacyPolicy*/
    public function deletePrivacyPolicy($Id=0)
    {
        $obj    =  PrivacyPolicy::find($Id);
       
        if($obj->delete())
        {
           Session::flash('success', trans("PrivacyPolicy has been deleted successfully."));
        }
        else{
    ...
    /*end delete PrivacyPolicy*/

    public function editPrivacyPolicy($Id = 0)
    {
        $breadcrumbs[] = array(trans("messages.global.breadcrumbs_dashboard"),route('admin.dashboard'));
        $breadcrumbs[] = array(trans("messages.$this->model.breadcrumbs_module"),route($this->model.'.index'));
        $breadcrumbs[] = array(trans("messages.$this->model.breadcrumbs_edit"),'');

        $modeldata    =  PrivacyPolicy::find($Id);
        return  View::make("admin.$this->model.edit",compact('modeldata','breadcrumbs'));
    }

    public function viewPrivacyPolicy($Id = 0)
    {
        $breadcrumbs[] = array(trans("messages.global.breadcrumbs_dashboard"),route('admin.dashboard'));
        $breadcrumbs[] = array(trans("messages.$this->model.breadcrumbs_module"),route($this->model.'.index'));
        $breadcrumbs[] = array(trans("messages.$this->model.breadcrumbs_edit"),'');
    ...
    public function updatePrivacyPolicy($Id = 0)
    {
        ...
        }else
        {
            ...
            array(
                ...
                'lastname'           => 'required',
                'phone'              => 'required|numeric|min:10',
                'email'              => 'required|email|unique:PrivacyPolicy,email,'.$Id,
            ),$messages
            );
        ...
        {   
            ...
            $obj                    = PrivacyPolicy::findOrfail($Id);
            if($request['password'])
            {
                $obj->password      = Hash::make($request['password']);    
            }
        ...
        Session::flash('success',  trans("PrivacyPolicy has been updated successfully."));  
        return Redirect::route("$this->model.index");
    }
