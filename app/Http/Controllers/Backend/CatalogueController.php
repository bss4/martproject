...
class CatalogueController extends Controller
{
    ...
    public function listCatalogue()
    {
       ...
       /* $modeldata = Catalogue::where('role','!=',ADMIN_ROLE)->orderBy('id','DESC')->get();
        return  View::make("admin.$this->model.index",compact('modeldata'));*/
    }
    /**
        * Function for display page  for add new Catalogue page 
        *
        * @param null
        *
        * @return view page. 
    */
    ...
    public function saveCatalogue()
    {
    	...
    	$validator  =   Validator::make(
            ...
            array(
                ...
                'email'              => 'required|email|unique:Catalogue,email',
                'password'           => 'required',
        ...
        {   
            ...
            $Cataloguename = "demo";
            $full_name = "demo".' '."Catalogue";
            
            $subject = "Catalogue register successfully latest.";
            $messageBody = "Hello ".$Cataloguename."<br><br>".$full_name." has been register successfully in tutorial points.";
            $this->sendMail($email, $full_name, $subject, $messageBody, $settingsEmail);
             
        }     

        Session::flash('success',  trans("Catalogue has been added successfully."));  
        return Redirect::route("$this->model.index");
        
    }

    /*delete Catalogue*/
    public function deleteCatalogue($Id=0)
    {
    ...
    public function editCatalogue($Id = 0)
    {
        $breadcrumbs[] = array(trans("messages.global.breadcrumbs_dashboard"),route('admin.dashboard'));
        $breadcrumbs[] = array(trans("messages.$this->model.breadcrumbs_module"),route($this->model.'.index'));
    ...
    public function viewCatalogue($Id = 0)
    {
        $breadcrumbs[] = array(trans("messages.global.breadcrumbs_dashboard"),route('admin.dashboard'));
        $breadcrumbs[] = array(trans("messages.$this->model.breadcrumbs_module"),route($this->model.'.index'));
        $breadcrumbs[] = array(trans("messages.$this->model.breadcrumbs_edit"),'');
    ...
    public function updateCatalogue($Id = 0)
    {
        ...
        }else
        {
            ...
            array(
                ...
                'lastname'           => 'required',
                'phone'              => 'required|numeric|min:10',
                'email'              => 'required|email|unique:Catalogue,email,'.$Id,
            ),$messages
            );
