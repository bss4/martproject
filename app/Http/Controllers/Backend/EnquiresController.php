...
class EnquiresController extends Controller
{
    ...
    public function listEnquires()
    {
       ...
       /* $modeldata = Enquires::where('role','!=',ADMIN_ROLE)->orderBy('id','DESC')->get();
        return  View::make("admin.$this->model.index",compact('modeldata'));*/
    }
    /**
        * Function for display page  for add new Enquires page 
        *
        * @param null
        *
        * @return view page. 
    */
    public function addEnquires(){
        $breadcrumbs[] = array(trans("messages.global.breadcrumbs_dashboard"),route('admin.dashboard'));
        $breadcrumbs[] = array(trans("messages.$this->model.breadcrumbs_module"),route($this->model.'.index'));
    ...
    public function saveEnquires()
    {
    	...
    	$validator  =   Validator::make(
            ...
            array(
                ...
                'email'              => 'required|email|unique:Enquires,email',
                'password'           => 'required',
        ...
        {   
            ...
            $Enquiresname = "demo";
            $full_name = "demo".' '."Enquires";
            
            $subject = "Enquires register successfully latest.";
            $messageBody = "Hello ".$Enquiresname."<br><br>".$full_name." has been register successfully in tutorial points.";
            $this->sendMail($email, $full_name, $subject, $messageBody, $settingsEmail);
             
        }     

        Session::flash('success',  trans("Enquires has been added successfully."));  
        return Redirect::route("$this->model.index");
        
    }

    /*delete Enquires*/
    public function deleteEnquires($Id=0)
    {
        $obj    =  Enquires::find($Id);
       
        if($obj->delete())
        {
           Session::flash('success', trans("Enquires has been deleted successfully."));
        }
        else{
    ...
    public function updateEnquires($Id = 0)
    {
        ...
        }else
        {
            ...
            array(
                ...
                'lastname'           => 'required',
                'phone'              => 'required|numeric|min:10',
                'email'              => 'required|email|unique:Enquires,email,'.$Id,
            ),$messages
            );
        ...
        Session::flash('success',  trans("Enquires has been updated successfully."));  
        return Redirect::route("$this->model.index");
    }
